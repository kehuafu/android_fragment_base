 ...
 * **版本:** v 1.0.0
 */
class TakePictureCallbackFactory(
    private val id: Int,
    private val action: String,
    private val targetFile: File,
    private val fileProvider: String,
    val isVideoAction: Boolean = action == MediaStore.ACTION_VIDEO_CAPTURE
) : CallbackFactory<File?> {
    ...
    private inner class TakePictureCallback : BaseCallback<File?>() {
        override fun onAttach(context: Context) {
            ...
            Log.e("PhotoSelector", "uri: $uri")
            intent.putExtra(MediaStore.EXTRA_OUTPUT, uri)
            OkActivityResult.startActivity(context as Activity, intent) { resultCode ->
                Log.e("PhotoSelector", "Activity.RESULT_OK: ${Activity.RESULT_OK}")
                Log.e("PhotoSelector", "resultCode: $resultCode")
                Log.e("PhotoSelector", "targetFile: ${targetFile.exists()}")
                Log.e("PhotoSelector", "contextOrNull: $contextOrNull")
                if (resultCode == Activity.RESULT_OK && targetFile.exists() && contextOrNull != null) {
                    ...
                    val picture = Picture(
                        ...
                        targetFile.length(),
                        if (isVideoAction) PictureType.VIDEO else PictureType.PHOTO,
                        duration.formatToDurationString(),
                    ...
                    MediaScannerConnection.scanFile(
                        ...
                        arrayOf(if (isVideoAction) "video/mp4" else "image/jpeg"),
                        null
                    )
                    onTakePictureFinished(picture)
                } else {
                    Log.e("PhotoSelector", "onAttach: null")
                    callback(null)
                }
            }
        }
